...
class TushareStockAdjFactorTask(BaseTushareTask):
    """Tushare 股票复权因子同步任务 - 流式处理版本"""

    def __init__(self, name: str = "stock_adj_factor", check_after_sync: bool = True,
    ...
    def fetch_data(self) -> pd.DataFrame:
        """
        从 Tushare 获取复权因子 - 流式处理版本
        每天获取后立即写入数据库，不再返回大 DataFrame
        """
        print("📥 从 Tushare 获取复权因子（流式处理）...")
