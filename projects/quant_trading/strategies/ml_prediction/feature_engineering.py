...
class FeatureEngineer:
    ...
    def _add_technical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.tech_config.use_price_patterns:
            ...
            df["engulfing"] = talib.CDLENGULFING(
                open=df["open"].values, high=high, low=low, close=close
            )
    ...
    def _add_market_features(self, df: pd.DataFrame, market_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["beta"] = self._calculate_rolling_beta(
            df["close"].pct_change(), market_aligned.pct_change(), window=60
        )
    ...
    def _calculate_rolling_beta(
        self, stock_returns: pd.Series, market_returns: pd.Series, window: int = 60
    ) -> pd.Series:
        """计算滚动Beta"""
    ...
    def create_target(
        self, df: pd.DataFrame, horizon: int = 1, target_type: str = "direction"
    ) -> pd.Series:
        """
        创建预测目标
    ...
    def prepare_train_test_split(
        self, df: pd.DataFrame, target: pd.Series, train_ratio: float = 0.7, val_ratio: float = 0.15
    ...
    def normalize_features(
        self, data_dict: Dict[str, pd.DataFrame], method: str = "zscore"
    ) -> Dict[str, pd.DataFrame]:
        """
        特征归一化
        ...
        # 保存归一化参数
        self.norm_params = (
            {"mean": mean, "std": std} if method == "zscore" else {"min": min_val, "max": max_val}
        )
