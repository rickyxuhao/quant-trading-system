...
ALTER TABLE t_stock_basic ADD CONSTRAINT chk_list_date_format 
    CHECK (list_date IS NULL OR list_date REGEXP '^[0-9]{8}$');
...
ALTER TABLE t_stock_basic ADD CONSTRAINT chk_delist_date_format 
    CHECK (delist_date IS NULL OR delist_date REGEXP '^[0-9]{8}$');
...
VALUES ('1.1.0', 'Added CHECK constraints for data integrity validation (MySQL)');
...
SELECT 
    ...
    CHECK_CLAUSE
FROM INFORMATION_SCHEMA.CHECK_CONSTRAINTS
WHERE TABLE_NAME = 't_stock_dailymarketdata';
*/
