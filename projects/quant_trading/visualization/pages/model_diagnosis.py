...
def render_feature_analysis():
    ...
    features = pd.DataFrame({
        'feature': ['MA5', 'MA20', 'RSI', 'MACD', 'Volatility', 'Volume_MA',
                    'Price_Momentum', 'ATR', 'Bollinger', 'CCI', 'ADX', 'Stochastic'],
        'importance': [0.145, 0.128, 0.115, 0.098, 0.087, 0.076,
                       0.072, 0.065, 0.058, 0.032, 0.024, 0.018],
        'category': ['Trend', 'Trend', 'Momentum', 'Momentum', 'Volatility', 'Volume',
                     'Momentum', 'Volatility', 'Volatility', 'Momentum', 'Trend', 'Momentum']
...
def render_ic_ir_analysis():
    ...
    with col2:
        ...
        ir_values = cum_ic / (np.arange(1, len(ic_values) + 1)) / (pd.Series(ic_values).expanding().std() + 1e-6)
        ...
        fig = px.line(ir_data, x='date', y='ir',
                     labels={'ir': 'IR', 'date': '日期'})
        fig.add_hline(y=0, line_dash="dash", line_color="gray")
...
def render_quantile_analysis():
    ...
    with col3:
        st.metric("多空收益", f"{long_short_return*100:.2f}%",
