...
def render_consecutive_analysis(trades_df: pd.DataFrame):
    ...
    # 计算连续序列
    trades_df_sorted['streak_group'] = (trades_df_sorted['is_win'] != trades_df_sorted['is_win'].shift()).cumsum()
    streaks = trades_df_sorted.groupby('streak_group').agg({
        'is_win': 'first',
        'pnl': 'count'
        ...
        if 'date' in stock_trades.columns:
            ...
            for _, sell in sell_trades.iterrows():
                ...
                if not matching_buys.empty:
                    ...
                    # 计算盈亏
                    total_buy = matching_buys.iloc[-1]['amount'] + matching_buys.iloc[-1]['total_cost']
