...
from projects.quant_trading.visualization.config import VizConfig, viz_config
...
def run_backtest():
    ...
    with st.spinner("正在运行回测..."):
        try:
            ...
            # 添加策略信息到结果
            mock_results['config']['strategy'] = strategy.get('name', 'Unknown') if strategy else 'Unknown'

            StateManager.set('backtest_results', mock_results)
            StateManager.set('last_run_params', {
                'strategy': strategy,
...
def main():
    ...
    if page == "策略绩效":
        ...
        from projects.quant_trading.visualization.pages.model_diagnosis import render_model_diagnosis_page
    ...
    with footer_col2:
        st.caption(f"© 2024 量化交易系统 | 数据更新时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
