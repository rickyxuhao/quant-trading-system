...
class LeadingStockStrategy(BaseStrategy):
    ...
    def generate_signals(
        self, data: Dict[str, pd.DataFrame], current_date: datetime, available_stocks: List[str]
    ) -> List[str]:
        """
        生成交易信号 - 按行业挑选龙头股
        ...
        for industry, stocks in industry_groups.items():
            ...
            for ts_code in stocks:
                ...
                financial_scores.append(
                    {"ts_code": ts_code, "score": score, "roe": roe, "industry": industry}
                )
            ...
            for leader in industry_leaders:
                print(
                    f"      - {leader['ts_code']}: ROE={leader['roe']*100:.1f}%, 得分={leader['score']:.2f}"
                )
...
if __name__ == "__main__":
    ...
    strategy = LeadingStockStrategy(top_n_per_industry=1, min_roe=0.10)
