...
数据来源: Tushare top10_holders API
"""
...
class Top10HoldersSync(BaseSyncTask):
    """前十大股东同步任务"""

    TABLE_NAME = "t_stock_top10_holders"
        ...
        for i, ts_code in enumerate(stock_list, 1):
            try:
                ...
                for period in periods:
                    try:
                        # 获取数据 - top10_holders 需要 ts_code 和 period 参数
...
def main():
    parser = create_base_parser("前十大股东同步 - t_stock_top10_holders")
    ...
    # 执行同步
    sync_task = Top10HoldersSync(config, db, client)
