...
class TestCalculateAdjustedPrice(unittest.TestCase):
    ...
    def test_forward_adjustment(self):
        """测试前复权计算"""
        result = calculate_adjusted_price(self.price_data, self.adj_factor_data, adj_type="forward")
    ...
    def test_backward_adjustment(self):
        ...
        result = calculate_adjusted_price(
            self.price_data, self.adj_factor_data, adj_type="backward"
        )
    ...
    def test_missing_adj_factor(self):
        ...
        result = calculate_adjusted_price(self.price_data, incomplete_adj, adj_type="forward")
    ...
    def test_missing_required_columns(self):
        """测试缺少必需列时的错误处理"""
        invalid_df = pd.DataFrame({"open": [100.0, 101.0], "close": [101.0, 102.0]})
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_single_row(self):
        ...
        single_adj = pd.DataFrame(
            {"ts_code": ["000001.SZ"], "trade_date": ["20240101"], "adj_factor": [1.5]}
        )
    ...
    def test_constant_adj_factor(self):
        ...
        pd.testing.assert_series_equal(
            result["adj_close"].reset_index(drop=True), price_data["close"].reset_index(drop=True)
        )
