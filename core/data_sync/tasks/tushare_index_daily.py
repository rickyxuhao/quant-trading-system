...
class TushareIndexDailyTask(BaseTushareTask):
    """Tushare 指数日线行情同步任务 - 流式处理版本"""

    # 沪深300指数代码
    INDEX_CODE = "000300.SH"

    def __init__(self, name: str = "index_daily", check_after_sync: bool = True,
    ...
    def fetch_data(self) -> pd.DataFrame:
        """
        从 Tushare 获取指数日线行情 - 流式处理版本
        每天获取后立即写入数据库，不再返回大 DataFrame
        """
        print("📥 从 Tushare 获取指数日线行情（流式处理）...")
        print(f"   目标指数: {self.INDEX_CODE}")
    ...
    def _get_last_trade_date(self) -> str:
        ...
        result = DatabaseManager.fetchone(
            self.db_name,
            f"SELECT MAX(trade_date) as max_date FROM {self.table_name} WHERE ts_code = %s",
            (self.INDEX_CODE,)
        )
